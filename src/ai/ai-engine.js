...
export function generatePassword(config) {
    ...
    const password = finalConfig.memorable
        ...
        : generateRandomPassword(finalConfig);
...
// ─────────────────────────────────────────────
export function explainPasswordStrength(analysis) {
...
function hasBrandImpersonation(domain) {
    const normalized = normalizeDomain(domain);
    return KNOWN_BRANDS.some(brand => {
        if (normalized.includes(brand) && !domain.toLowerCase().startsWith(brand))
            return true;
        if (domain !== normalized && normalized.includes(brand))
            return true;
        return false;
    });
}
...
export function processAIMessage(userMessage, context) {
    const msg = userMessage.toLowerCase().trim();
    // Password generation
    if (msg.includes('generat') || msg.includes('creat') || msg.includes('make') || msg.includes('suggest')) {
        let context_type = 'general';
        if (msg.includes('bank') || msg.includes('financ'))
            context_type = 'banking';
        else if (msg.includes('social') || msg.includes('instagram') || msg.includes('twitter'))
            context_type = 'social';
